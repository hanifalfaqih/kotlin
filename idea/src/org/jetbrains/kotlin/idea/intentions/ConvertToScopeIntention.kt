...
sealed class ConvertToScopeIntention(
    ...
    override fun applyTo(element: KtExpression, editor: Editor?) {
        if (!tryApplyTo(element, dryRun = false)) {
            CommonRefactoringUtil.showErrorHint(element.project, editor, "Could't apply the refactoring", text, null)
        }
    }
        ...
        return when (scopeFunction) {
            ALSO, APPLY -> {
                ...
                val lastTarget = if (firstTarget === this)
                    lastTargetSequence.lastOrNull()
                else
                    lastTargetSequence.lastWithPersistedElementOrNull(elementShouldPersist = this)
            ...
            RUN, WITH -> {
                ...
                val lastTarget = collectTargetElements(referenceName, forward = true).lastOrNull() ?: this
        ...
        val (scopeFunctionCall, callExpression) = when (scopeFunction) {
            ALSO, APPLY -> {
                ...
                val initializerPattern = when (initializer) {
                    ...
                    is KtParenthesizedExpression -> initializer.text
